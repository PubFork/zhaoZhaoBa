...
static NSTimeInterval kQrLineanimateDuration = 1;
...
- (void)initQRLine {
    ...
    UIImageView * imageView = [[UIImageView alloc] initWithFrame:CGRectMake(0, 0, 254, 254)];
    imageView.center = CGPointMake(kScreenWidth / 2, kScreenHeight / 2 - 100);
    imageView.image = kImage(@"Scan_Success");
    [self addSubview:imageView];
    
    
    qrLine  = [[UIImageView alloc] initWithFrame:CGRectMake(0, imageView.origin.y - 80, 226, 137)];
    qrLine.image = [UIImage imageNamed:@"qr_scan_line"];
//    qrLine.backgroundColor = [UIColor orangeColor];
    qrLine.contentMode = UIViewContentModeScaleAspectFill;
    qrLine.center = CGPointMake(kScreenWidth / 2, qrLine.centerY);
...
- (void)show {
    ...
    [UIView animateWithDuration:kQrLineanimateDuration animations:^{
        ...
        rect.origin.y = kScreenHeight / 2 - 125;
        qrLine.frame = rect;
